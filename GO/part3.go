...
func lookup(codes []int, palette [][3]uint8) []uint8 {
    fmt.Println("WARNING: Please copy sequential lookup code from Part1 here")
     newarr := make([]uint8,3*len(codes))
    for i := 0 ; i < len(codes); i++{
            for j:= 2 ; j >= 0 ; j--{
                if codes[i] < len(palette){
                    newarr[i*3+j] = palette[(codes[i])][j]
            }
        }
    }
...
and you will produce color codes that are then used by the lookup function
you wrote in part 1/2. You will get a dictionary (actually a 2D array)
that you need to index using the GIF codes.
However, unlike parts 1/2 the number of color codes against a GIF code is variable.
So the size of the resulting array is not known in advance.
To avoid repeated allocations (with make or append), you are required to first calculate
the size of the output array (by going over all GIF codes and finding the length of color
codes against each GIF code) and then do a single allocation (using make) and finally traverse to fill in the color codes.
*/
func decompress(codes []int, dict [][]int) []int {

    le := 0
    for i := 0 ; i < len(codes); i++{  
            if(len(dict[codes[i]]) > 0){
                le = le + len(dict[codes[i]])
            } 
        }
...
In this part, you are to parallelize the above GIF decoding.
It is easy to divide the task (like Part 2) but the complication is
that we do not know where should the goroutine working on the right half of GIF codes
start writing in the output array.
Thus we use a PrefixSum kind of solution.
We ask both halves how much space they need and then tell the total to the parent.
The parent will then give a big enough slice.
Since the left half told us how much space it will occupy,
we can send the correct sub-slices to the left and right goroutines.
...
func parallel_decompress(codes []int, dict[][]int, size chan<- int, slice <-chan []int) {
    ...
    if len(codes) <= 16{
        ...
        slice2 <- y
        go parallel_decompress(codes[:mid], dict, size1, slice1)
        go parallel_decompress(codes[mid:], dict, size2, slice2)
        ss := <-size1
        bb := <-size2
        fmt.Println("hello")
        ...
        //go parallel_decompress2(codes[:mid], dict, ss, slice1)
        //go parallel_decompress2(codes[mid:], dict, bb, slice2)

        size<- ss + bb // tell parent size
    }
